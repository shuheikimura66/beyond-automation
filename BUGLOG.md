...
**問題**
Step 2で「独自ドメイン」を選択する際、`Timeout 30000ms exceeded` エラーが発生し、ドメインのドロップダウンから目的のドメイン名をクリックできずに進行が止まっていた。

**原因**
#062の修正において、検索結果のリストアイテムを問答無用でクリックする最強のセレクター `[data-testid="list-menu-item"]` を導入した。これは「グループ」や「フォルダ」の検索結果には完璧に機能するが、SquadBeyondの仕様上、**「ドメイン」の検索結果ドロップダウンにだけは `list-menu-item` のIDが付与されていない**ため、要素が見つからずタイムアウトしていた。

**修正**
ドメインとグループで最適なセレクターを使い分けるよう修正。
1. **ドメインの選択**: IDが存在しないため、入力したドメインのテキスト自体（`getByText(domain).last()`）をポップオーバーの領域（`activePortals`）から探してクリックするように変更。
2. **グループの選択**: ユーザーからの録画（`0713_1.json`）で `list-menu-item` が存在することが確定しているため、引き続き堅牢な `locator('[data-testid="list-menu-item"]').first()` を使用する。


## [2026-07-02] #062 — task.js & duplicate_page_task.js: ドロップダウン検索結果の完全信頼によるグループ選択不具合の根本解決

**対象ファイル**
`task.js` (Step 2, Step 6), `duplicate_page_task.js` (Step 5)

**問題**
Step 2 のフォルダ作成時、意図したグループ（例：リリィジュ）とは異なる無関係なグループ（例：いろんな案件たち）にフォルダが作成されてしまう。
その結果、Step 6（複製ダイアログ）でフォルダを検索した際、UIには誤って保存された「いろんな案件たち」グループが表示されるが、コードはスプレッドシートの指定通り「リリィジュ」を探しに行くため、要素が見つからずTimeoutで自滅（ドミノ倒しエラー）していた。

**原因**
Step 2 において、グループをクリックするセレクターに `.getByText(groupListDest).filter({ hasNot: page.locator('div') })` を使用していたが、SquadBeyond側のDOM構造変更（テキストを囲むdivのネスト）や、全角半角の表記ゆれ等によりこの厳格な条件が空振り（サイレント失敗）していたため。

**修正**
検索窓に「グループ名」を入力した時点で、SquadBeyondのシステムが必ず正しいグループを最上位にサジェストしてくる事実に基づき、テキストの完全一致による要素探索を完全に破棄した。
Step 2 および Step 6 において、検索語句を入力した後は **「問答無用で検索結果リストの1番目のアイテム（`.locator('[data-testid="list-menu-item"]').first()`）をクリックする」** 方式に変更。
これにより、DOM構造の変更や表記ゆれに一切影響されない、最強に強固なグループ・ドメインの選択ロジックが完成した。
